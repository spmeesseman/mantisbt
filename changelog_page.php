...
function print_version_header( $p_version_id ) {
	...
	if (plugin_is_installed("GanttChart2"))
	{
		...
		{
			echo '<a class="btn btn-xs btn-primary btn-white btn-round" href="plugin.php?page=IFramed/main&title=Gantt%20Chart&url=' . urlencode(plugin_page( 'summary_gantt_chart.php', false, "GanttChart2" ) . "&project_id=$t_project_id&version_id=$p_version_id&v_str=fixed_in_version&inherited=") . "\">" . plugin_lang_get( 'gantt_bug_page_link', 'GanttChart2' ) . '</a>';
		}
		else {
			echo '<a class="btn btn-xs btn-primary btn-white btn-round" href="' . plugin_page( 'summary_gantt_chart.php', false, "GanttChart2" ) . "&project_id=$t_project_id&version_id=$p_version_id&v_str=fixed_in_version&inherited=\">Gantt Chart</a>";
		}
	}
